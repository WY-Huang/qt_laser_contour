...
void MyPclFunction::cvpoint3f_to_oneline_pclclould(std::vector<cv::Point3f>cv_cloud, float x,
...
void MyPclFunction::float_to_oneline_pclclould(float *f_data,int f_datanum,float y,pcl::PointCloud<pcl::PointXYZRGB>::Ptr *point_cloud_ptr_Out)
{
    pcl::PointXYZRGB point;
...
void MyPclFunction::updata_color_pclclould(pcl::PointCloud<pcl::PointXYZRGB>::Ptr *point_cloud_ptr_In,pcl::PointCloud<pcl::PointXYZRGB>::Ptr *point_cloud_ptr_Out)
{
...
void MyPclFunction::pclclould_to_rangeImage(pcl::PointCloud<pcl::PointXYZRGB>::Ptr *point_cloud_ptr_In,pcl::RangeImage *rangeImage)
{
...
void MyPclFunction::cv_f32deepimg_to_show8deepimg(cv::Mat f32_deepimg,cv::Mat *f8_deepimg)
{
    int i,j;
...
void MyPclFunction::pointCloud2imgI(pcl::PointCloud<pcl::PointXYZRGB>::Ptr *point_cloud_ptr_In, cv::Mat *f8_deepimg,double resolution)
{
  ...
  // 获取点云的最大最小 x、y点的坐标
  for (int i = 0; i < (int)(*point_cloud_ptr_In)->points.size(); i++)
  {
...
Int8 MyPclFunction::Myfixfdata(float *dataIn_Out,Uint8 *mask,Int32 num)
{
    Int32 n,m;
    Int32 nST,nED;
...
void MyPclFunction::addpoint_image(cv::Mat *f8_deepimg,int coldis,int rowdis)
{
  if(coldis>0)
  {
